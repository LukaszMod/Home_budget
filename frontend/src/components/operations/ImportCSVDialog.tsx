...
const ImportCSVDialog: React.FC<ImportCSVDialogProps> = ({
  ...
  const findMatchingAccount = (inputValue: string) => {
    ...
    return accounts.find((account) => {
      const candidates = [account.name, account.account_number, account.accountNumber].filter(
  ...
  const convertRowToOperation = (row: CSVRow, mapping: ColumnMapping): any => {
    ...
    const descriptionHeader =
      mapping.description !== undefined ? headers[mapping.description] : undefined;
    const dateHeader = mapping.date !== undefined ? headers[mapping.date] : undefined;
    const sourceAccountHeader =
      mapping.sourceAccount !== undefined ? headers[mapping.sourceAccount] : undefined;
    const categoryHeader = mapping.category !== undefined ? headers[mapping.category] : undefined;
    const operationTypeHeader =
      mapping.operationType !== undefined ? headers[mapping.operationType] : undefined;
    ...
    if (row['__asset_id__']) {
      ...
      const accountValue = String(row[sourceAccountHeader] || '').trim();
      if (accountValue) {
        const account = findMatchingAccount(accountValue);
        console.log('Matching account for', accountValue, ':', account);
        if (account) {
    ...
    if (row['__operation_type__']) {
      ...
      operationType = type.includes('income') || type.includes('przychód') ? 'income' : 'expense';
    } else {
  ...
  const handleImport = async () => {
    try {
      ...
      if (validOperations.length === 0) {
        notify(
          t('import.errors.noValidOperations', 'Brak poprawnych operacji do zaimportowania'),
          'error'
        );
        setIsImporting(false);
        return;
  ...
  return (
    <Dialog open={open} onClose={handleClose} maxWidth="xl" fullWidth>
      <DialogTitle>{t('import.title', 'Import operacji z CSV')}</DialogTitle>
      <DialogContent>
        <Box sx={{ mb: 3 }}>
        ...
        {activeStep === 0 && <UploadCSVStep onCSVParsed={handleCSVParsed} onError={setError} />}
      ...
      <DialogActions>
        ...
        {activeStep > 0 && <Button onClick={handleBack}>{t('common.back', 'Wstecz')}</Button>}
        {activeStep === 1 && (
          <Button onClick={handleMappingNext} variant="contained" disabled={!isMappingValid}>
            {t('common.next', 'Dalej')}
          </Button>
        )}
        {activeStep === 2 && (
