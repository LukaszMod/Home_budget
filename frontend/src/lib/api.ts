...
export const createAssetValuation = async (
  payload: CreateAssetValuationPayload
...
export const classifyUncategorizedOperations = async (): Promise<{
...
export const splitOperation = async (
  id: number,
...
export const updateBudget = async ( payload: CreateBudgetPayload[]): Promise<Budget> => {
  return fetchJson(`${API}/budgets/update`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(payload),
  });
...
export const updateImportTemplate = async (
  id: number,
  payload: Partial<CreateImportTemplatePayload>
): Promise<ImportTemplate> => {
